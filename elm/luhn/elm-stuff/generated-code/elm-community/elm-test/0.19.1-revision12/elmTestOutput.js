...
var $author$project$Luhn$doubleEverySecond = F2(
	function (index, elem) {
		var isEven = !((index + 1) % 2);
		if (isEven) {
			var doubled = elem * 2;
...
var $author$project$Luhn$valid = function (input) {
	var inputWithoutSpaces = A3($elm$core$String$replace, ' ', '', input);
	var numList = A2(
		$elm$core$List$filterMap,
		$elm$core$String$toInt,
		A2($elm$core$String$split, '', inputWithoutSpaces));
	var sum = $elm$core$List$sum(
		A2(
			$elm$core$List$indexedMap,
			$author$project$Luhn$doubleEverySecond,
			$elm$core$List$reverse(numList)));
	var origLen = $elm$core$String$length(inputWithoutSpaces);
	var digitsLen = $elm$core$List$length(numList);
	return ((digitsLen <= 1) || (!_Utils_eq(digitsLen, origLen))) ? false : (!(sum % 10));
};
		...
		[
			...
			function (_v4) {
				return A2(
					$elm_explorations$test$Expect$equal,
					true,
					$author$project$Luhn$valid('055 444 285'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v5) {
				return A2(
					$elm_explorations$test$Expect$equal,
					false,
					$author$project$Luhn$valid('055 444 286'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			'invalid credit card',
			function (_v6) {
				return A2(
					$elm_explorations$test$Expect$equal,
					false,
					$author$project$Luhn$valid('8273 1232 7352 0569'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v7) {
				return A2(
					$elm_explorations$test$Expect$equal,
					false,
					$author$project$Luhn$valid('1 2345 6789 1234 5678 9013'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v8) {
				return A2(
					$elm_explorations$test$Expect$equal,
					false,
					$author$project$Luhn$valid('055a 444 285'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v12) {
				return A2(
					$elm_explorations$test$Expect$equal,
					true,
					$author$project$Luhn$valid('0000 0'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v14) {
				return A2(
					$elm_explorations$test$Expect$equal,
					true,
					$author$project$Luhn$valid('9999999999 9999999999 9999999999 9999999999'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			...
			function (_v15) {
				return A2(
					$elm_explorations$test$Expect$equal,
					true,
					$author$project$Luhn$valid('109'));
			}),
			A2(
			$elm_explorations$test$Test$test,
			'numeric, non-space char in the middle with a sum that\'s divisible by 10 isn\'t allowed',
			function (_v16) {
				return A2(
					$elm_explorations$test$Expect$equal,
					false,
					$author$project$Luhn$valid('59%59'));
			})
		]));
var $author$project$Test$Generated$Main$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		...
		paths: _List_fromArray(
			['/home/gasks/workspace/exercism/elm/luhn/tests/Tests.elm']),
		processes: 8,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: 100,
		seed: 227380587810586
	},
	_List_fromArray(
		[
			_Utils_Tuple2(
...
var pipeFilename = "/tmp/elm_test-1084760.sock";
var net = require('net'),
